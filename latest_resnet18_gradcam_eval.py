...
def main():
    ...
    # 2. Instantiate the ResNet-18 model and modify the final layer for 2 classes
    model = resnet18()
    num_ftrs = model.fc.in_features
    model.fc = nn.Linear(num_ftrs, 2)
    model = model.to(device)
    
    checkpoint_path = 'resnet18_checkpoint/best_resnet18_model.pth'
    ...
    # 3. Specify the target layer for Grad-CAM in ResNet-18
    ...
    plt.subplot(1, 2, 2)
    plt.title("ResNet-18 Grad-CAM Heatmap")
    plt.imshow(visualization)
    plt.axis('off')
    
    plt.tight_layout()
    
    # Save the results with a specific prefix for ResNet
    output_path = os.path.join(output_dir, f"resnet18_heatmap_{image_name}")
